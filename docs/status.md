...
| JD parsing | `done` | scoring | 공고 후보를 title/company/location/deadline/requirements/responsibilities 등 구조화된 snapshot으로 변환한다. | `internal parse_candidates` | `test_unknown_deadline_is_uncertain_not_expired`<br />`test_each_report_posting_has_public_safe_report_fields` |
