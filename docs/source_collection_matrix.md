...
| NAVER Careers | Company careers candidate | `deferred` | `null` | [`public_http`] | V1 4-candidate gate에서 검토했지만 generic `PublicJobsHttpAdapter` live probe 결과 후보 0개라 enable하지 않습니다. | `docs/source_access_reviews.md#company-careers-v1-gate`, `config/live_sources.sample.json`, `tests/test_dry_run.py::SourceCandidateSelectionTests`; needs NAVER-specific `/rcrt/loadJobList.do` + detail parser | source-specific parser가 non-zero candidate evidence를 만들 때 enable |
| Kakao Careers | Company careers candidate | `deferred` | `null` | [`public_http`] | V1 4-candidate gate에서 검토했지만 SPA shell만 public HTTP로 보이고 generic adapter live probe 결과 후보 0개라 enable하지 않습니다. | `docs/source_access_reviews.md#company-careers-v1-gate`, `config/live_sources.sample.json`, `tests/test_dry_run.py::SourceCandidateSelectionTests`; needs Kakao-specific SPA/API + detail parser | source-specific parser가 non-zero candidate evidence를 만들 때 enable |
| LINE Careers | Company careers fallback | `deferred` | `null` | [`public_http`] | V1 fallback으로 보존합니다. | `docs/source_access_reviews.md#company-careers-v1-gate` | selected source 실패 시 재검토 |
| Coupang Careers | Company careers fallback | `deferred` | `null` | [`public_http`] | V1 fallback으로 보존합니다. | `docs/source_access_reviews.md#company-careers-v1-gate` | selected source 실패 시 재검토 |
...
| Company careers | Expansion | `deferred` | `null` | [`public_http`] | Aggregate placeholder only. Reviewed concrete candidates are `naver_careers`, `kakao_careers`, `line_careers`, and `coupang_careers`; none currently count as enabled V1 sources. | per-domain rows above | Do not count this placeholder as an enabled source |
...
현재 V1 target enabled source는 `jobkorea`, `saramin`, `wanted`, `jumpit`, `rallit`, `rocketpunch`입니다. `company_careers` placeholder와 `naver_careers`/`kakao_careers`/`line_careers`/`coupang_careers`는 후보/보류 상태이며, `linkedin`은 제외입니다.
