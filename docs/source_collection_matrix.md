...
| Wanted | Core | `enabled` | `public_http` | [] | 공개 검색/목록에서 `/wd/` detail 링크를 발견하고 detail HTML을 public HTTP parser로 수집합니다. | `tests/test_wanted_adapter.py::test_wanted_adapter_collects_public_detail_body_without_manual_payload`, `tests/test_wanted_adapter.py::test_wanted_adapter_discovers_public_wd_urls_from_listing`, `artifacts/ultragoal_g003/wanted_browser_transcript.json`; adapter `WantedAdapter` | 검색/detail shape 변경 시 fixture/parser 갱신 |
