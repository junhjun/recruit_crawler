...
Decision: Manual/export/user-visible capture, API/partner, public HTTP, and OCR remain non-target. User-directed policy override enables only no-human browser automation for the jobs page. Browser QA confirmed RocketPunch links do not leave the list shell; clicking a card changes the URL to `/en/jobs?selectedJobId=<id>` and opens a detail pane containing Responsibilities/Qualifications/deadline. `RocketPunchBrowserAutomationAdapter` therefore retains and fetches direct `/en/jobs/<id>` hrefs, synthesizing a `selectedJobId` URL only when a direct href is unavailable. If no card href is present, it falls back to listing URL plus a synthetic posting id.